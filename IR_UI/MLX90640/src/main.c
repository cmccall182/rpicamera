...
int main() {
	...
	printf("now beginning measurements\n");
	char filepath[100];
	char date[20];
	...
	while (1) {
		for(int8_t i = 0; i < 2; i++)
		{
			...
			float Tr = Ta - 8; //datasheet (or ta - 8)
			float emiss = 0.95; //datasheet
		...
		usleep(5000); //wait 5ms btw measurements (from datasheet)
	}
}
